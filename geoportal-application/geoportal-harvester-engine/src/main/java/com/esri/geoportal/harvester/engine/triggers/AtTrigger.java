        ...
        if (!"*".equals(dayOfTheWeek)) {
          ...
          Arrays.asList(dayOfTheWeek.split(",")).forEach(str->{
            try {
              ...
              LOG.warn(escapeHtml4(crlfSanitize(String.format("Invalid day of the week definition: %s", str))), ex);
            }
          });
        ...
        if (!"*".equals(weekOfTheMonth)) {
          ...
          Arrays.asList(weekOfTheMonth.split(",")).forEach(str->{
            try {
              ...
              LOG.warn(escapeHtml4(crlfSanitize(String.format("Invalid week of the month definition: %s", str))), ex);
            }
          });
          predicates.add((d)->pred.stream().map(p->p.test(d)).anyMatch(b->b==true));
        }
        ...
        if (!"*".equals(monthOfTheYear)) {
          ...
          Arrays.asList(monthOfTheYear.split(",")).forEach(str->{
            try {
              ...
              LOG.warn(escapeHtml4(crlfSanitize(String.format("Invalid month definition: %s", str))), ex);
            }
          });
          predicates.add((d)->pred.stream().map(p->p.test(d)).anyMatch(b->b==true));
        }
